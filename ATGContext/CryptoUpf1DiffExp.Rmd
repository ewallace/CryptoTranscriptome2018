...
ggplot(data=diffexp,
    ...
    scale_x_continuous("Log2 fold change, upf1Δ",
...
```{r volcano_select,dependson="load_data",fig.width=4,fig.height=2.5,results="show"}

library(ggnewscale)

genelist_u <- c("CNM01300", "CNM02470", "CNF00330", "CNG04240", "CNL04930","CND01050")
# volcano_diff <-
diffexp_u <- 
    diffexp %>%
    filter(Gene %in% genelist_u) %>%
    mutate(Geneshort = str_sub(Gene,start=3L),
           GeneFac = factor(Gene,levels=genelist_u))

ggplot(data=diffexp,
       aes(x=log2FoldChange,y=-log10(pvalue))) + 
    geom_vline(xintercept=0,size=0.3,colour="grey50") +
    geom_point(size=0.3,colour="grey50") +
    scale_y_continuous(expand=c(0.01,0.01)) +
    scale_x_continuous("Log2 fold change, upf1Δ",
                      limits=c(-6.1,6.1),expand=c(0,0),breaks=c(-6,-3,-2,-1,0,1,2,3,6),
                      oob=scales::squish) +
    coord_cartesian(clip="off") + 
    theme(legend.position="none") +
    geom_point(data=diffexp_u,size=1,
                    aes(label=Geneshort,color=GeneFac)) + 
    geom_text_repel(data=diffexp_u,
                    point.padding=0.2,force=0.4,
                    aes(label=Geneshort,color=GeneFac)) +
    scale_color_brewer(type="qual",palette=2)

diffexp %>%
    filter(Gene %in% c(genelist_u,"CNB05380")) %>%
    arrange(desc(log2FoldChange)) %>%
    select(Gene,WT,upf1E30,log2FoldChange,pvalue)
```
...
ggplot(data=udiff,
    ...
    scale_y_continuous("Log2 fold change, upf1Δ",
                      limits=c(-5,5),expand=c(0.02,0.02),
                      oob=scales::squish) +
    labs(x="uAUG count")
```

## Fold Change vs uATG count, ecdf
...
## Fold Change vs uATG count, KS test

```{r uATG_ct_kstest, dependson="load_data", fig.width=4,fig.height=1.8, results="show"}
# udiff
ks.test(x = udiff %>% filter(uATGCtC=="0") %>% .$log2FoldChange,
        y = udiff %>% filter(uATGCtC=="1") %>% .$log2FoldChange,
        alternative = "greater")

ks.test(x = udiff %>% filter(uATGCtC=="1") %>% .$log2FoldChange,
        y = udiff %>% filter(uATGCtC=="2+") %>% .$log2FoldChange,
        alternative = "greater")
...
```{r uATG_ctmin20_kstest, dependson="load_data", fig.width=4,fig.height=1.8, results="show"}
# udiff %>%
#   select(Gene,log2FoldChange,uATGfac)

ks.test(x = udiff %>% filter(uATGfac=="0") %>% .$log2FoldChange,
        y = udiff %>% filter(uATGfac=="1+\nclose") %>% .$log2FoldChange,
        alternative = "greater")
...
udiffsc1 <- inner_join(
    udiff %>%
        select(Gene,log2FoldChange,uATGCt) %>%
        filter(uATGCt==1),
    scores %>%
...
```{r uATG_score_kstest, dependson="load_scores", fig.width=4,fig.height=1.8, results="show"}
# udiffsc1
ks.test(x = udiffsc1 %>% filter(u1.scorekw < 0.7) %>% .$log2FoldChange,
        y = udiffsc1 %>% filter(u1.scorekw > 0.9) %>% .$log2FoldChange,
        alternative = "greater")

ks.test(x = udiffsc1 %>% filter(u1.scorekw < 0.8) %>% .$log2FoldChange,
        y = udiffsc1 %>% filter(u1.scorekw > 0.8) %>% .$log2FoldChange,
        alternative = "greater")
```
