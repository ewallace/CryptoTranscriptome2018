...
names(CDS) <- names(CDS) %>% str_sub(end=8L)
...
```{r ccposframe_ascore,dependson="load_cds",results="show"}
scores_JEC21 <- 
    read_tsv("data_out/scores_kozak_JEC21.txt",comment="#") %>%
    select(Gene,aATG.scorekn)

CDS_hiscore <- scores_JEC21 %>%
    filter(aATG.scorekn >= 0.85) %>%
    .$Gene %>%
    intersect(names(CDS)) %>%
    CDS[.]

CDS_loscore <- scores_JEC21 %>%
    filter(aATG.scorekn < 0.85) %>%
    .$Gene %>%
    intersect(names(CDS)) %>%
    CDS[.]

ccposframe_score <- bind_rows(
    CDS_hiscore %>% 
        subseq(start = 1) %>%
        countCodonsPos %>%
        mutate(Frame=0,ascore="hi"),
    CDS_hiscore %>% 
        subseq(start = 2) %>%
        countCodonsPos %>%
        mutate(Frame=1,ascore="hi"),
    CDS_hiscore %>% 
        subseq(start = 3) %>%
        countCodonsPos %>%
        mutate(Frame=2,ascore="hi"),
    CDS_loscore %>% 
        subseq(start = 1) %>%
        countCodonsPos %>%
        mutate(Frame=0,ascore="lo"),
    CDS_loscore %>% 
        subseq(start = 2) %>%
        countCodonsPos %>%
        mutate(Frame=1,ascore="lo"),
    CDS_loscore %>% 
        subseq(start = 3) %>%
        countCodonsPos %>%
        mutate(Frame=2,ascore="lo")) %>%
    ungroup %>%
    mutate(Frame=factor(Frame))
ccposframe_score
```
...
```{r plot_ccposframe_ATG_ascore,dependson="ccposframe_ascore",fig.height=3,fig.width=5}
ggplot(data=ccposframe_score %>% filter(Codon=="ATG"),
       aes(x=Pos,y=Freq,colour=ascore)) +
    geom_line(aes(group=ascore),size=0.7) + 
    scale_x_continuous("Position",limits=c(2,100),expand=c(0,1)) + 
    scale_y_continuous("ATG Frequency",limits=c(0,0.04),expand=c(0,0)) +
    scale_colour_brewer(palette="Set1") +
    facet_wrap(~Frame,ncol=3) +
    panel_border()
```
