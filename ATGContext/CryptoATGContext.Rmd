...
keepcols_ctbl <- c("Gene", "aATG.context", "aATG.pos",
...
aAUG_hi_context_JEC21_bare <- 
    ggseqlogo(data=filter(ctbl_big_JEC21,Gene %in% hiTrans_JEC21)$aATG.context,
              namespace="TCAG") + 
    theme_nothing()
aAUG_hi_context_JEC21_bare

aAUG_hi_context_JEC21 <- aAUG_hi_context_JEC21_bare  +
    expand_limits(y=-1.1) + 
    # annotate(geom="segment",x=8.5,xend=15.5,y=-0.1,yend=-0.1,size=1,colour="grey30") +
    # annotate(geom="text",label="narrow",x=12,y=-0.1,size=4, vjust=1,colour="grey30") +
    annotate(geom="segment",x=2.5,xend=15.5,y=-0.1,yend=-0.1,size=0.8,colour="grey50") +
    annotate(geom="text",label="+1",x=13,y=-0.2,hjust=0.5,vjust=1.2,colour="black")  # +
    #annotate(geom="text",label="wide",x=9,y=-0.2,size=4, vjust=1.15,colour="grey50") 
```

```{r a_hi_context_svg_JEC21,fig.width=3,fig.height=0.8,dev="svg"}
ggseqlogo(data=filter(ctbl_big_JEC21,Gene %in% hiTrans_JEC21)$aATG.context,
              namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
    annotate(geom="text",label="C. deneoformans",
             x=2.5,y=2,size=4,vjust=1,hjust=0,family="Arial",fontface = "italic")
```
...
```{r cytoribo_context_JEC21_svg,dependson="cytoribo_context",fig.width=3,fig.height=0.55,dev="svg"}
ggseqlogo(data=filter(ctbl_big_JEC21,Gene %in% cytoribolist_JEC21)$aATG.context,
              namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
...
plot_4ATGscoresw_dens(scores_JEC21,hiTrans_JEC21,xlab="Kozak score")
```

```{r plot_cumscores_w_JEC21, dependson="calc_scores_JEC21", fig.width=5,fig.height=1.8}
plot_cumscores_w_JEC21 <- 
    plot_4ATGscoresw_ecdf(scores_JEC21,hiTrans_JEC21,xlab="Kozak score")
plot_cumscores_w_JEC21
```
...
# plot_4ATGscoresw_dens(scores_JEC21,hiTrans_JEC21,xlab="wide Kozak score, JEC21",
#                       u1=FALSE,d2=TRUE)
...
plot_ATGdiffs_dens(scores_JEC21,
                   xlab=expression(paste("Kozak score diff. ", d[1],"AUG - aAUG")),
                   difftype="downstream")

dvsaATG_cumscores_JEC21 <- 
    plot_ATGdiffs_ecdf(scores_JEC21,
                       xlab=expression(paste("Kozak score diff. ", d[1],"AUG - aAUG")),
                       difftype="downstream")
...
plot_ATGdiffs_dens(scores_JEC21,
                   xlab=expression(paste("Kozak score diff. ", u[1],"AUG - aAUG")),
                   difftype="upstream")

uvsaATG_cumscores_JEC21 <- 
  plot_ATGdiffs_ecdf(scores_JEC21,
                     xlab=expression(paste("Kozak score diff. ", u[1],"AUG - aAUG")),
                     difftype="upstream") +
  scale_colour_hue(breaks=c("narrow","wide"),
                   labels=c("narrow (-4)","wide (-10)"))
uvsaATG_cumscores_JEC21
```
...
ggplot(data=scores_JEC21 %>% 
    ...
    labs(x="aAUG score, narrow",
         y="d1AUG score, narrow")
```
...
```{r dvsaATG_scoresw_JEC21, dependson="calc_scores_JEC21", fig.width=4.5,fig.height=3.5}
highdiffw_dvsaATG_JEC21 <- scores_JEC21 %>%
    arrange(desc(d1vsaw)) %>%
    filter(d1.scorekw - aATG.scorekw > 0.1) %>%
    # head(n=330) %>%
...
ggplot(data=scores_JEC21 %>% 
    ...
    geom_diagline() +
    geom_point(aes(size="other",colour="other")) +
    geom_point(data=highdiffw_dvsaATG_JEC21,
               aes(size="d1AUG hi",colour="d1AUG hi")) +
    geom_point(data=scores_JEC21 %>% 
                   filter(Gene %in% hiTrans_JEC21),
               aes(size="hiTrans",colour="hiTrans")) +
    geom_point(data=highdiffw_dvsaATG_JEC21 %>% 
                   filter(Gene %in% hiTrans_JEC21),
               colour="purple4",size=1.3) +
    scale_size_manual("Gene",breaks=c("hiTrans","d1AUG hi","other"),
                      values=c("other"=0.5,"hiTrans"=1,"d1AUG hi"=1)) + 
    scale_colour_manual("Gene",breaks=c("hiTrans","d1AUG hi","other"),
                        values=c("other"="grey50","hiTrans"="blue","d1AUG hi"="red")) +
    stat_smooth(method="gam",formula=y~s(x,k=4),size=0.7,
                colour="green3",fill="green3") + 
    coord_equal() + 
    labs(x="aAUG Kozak score",
         y=expression(paste(d[1],"AUG Kozak score")))
```
...
ggplot(data=scores_JEC21_Cutn,
    ...
    geom_diagline() +
    geom_point(size=0.5,colour="grey50") +
    stat_smooth(method="gam",formula=y~s(x,k=4),fill="blue") + 
...
## Genes with unusual dATG vs aAUG wide score

Saved to file *dvsaATG_highdiffw_enoughRNA_JEC21.txt*.

```{r highdiffw_dvsaATG_enoughRNA_JEC21,dependson="dvsaATG_scoresn_JEC21",results="show"}
highdiffw_dvsaATG_enoughRNA_JEC21 <- 
    highdiffw_dvsaATG_JEC21 %>%
    left_join( ctbl_big_JEC21 %>% select(Gene,d1.posATG,d1.frame) ) %>%
    filter(Gene %in% enoughRNA_JEC21) %>%
    mutate(d1.frame = (d1.frame == 0) %>% 
               factor(levels=c(TRUE,FALSE),labels=c("In","Out") ) ) %>%
    select(Gene,aATG.scorekw,d1.scorekw,d1.posATG,d1.frame)

highdiffw_dvsaATG_enoughRNA_JEC21


cat(
    "# dvsaATG_highdiffw_enoughRNA_JEC21.txt
# Genes with in-frame high-score dATGs
# In C. deneoformans JEC21:
#   - difference, aATG.scorekw - d1.scorekw > 0.1
#   - in top 50% by RNA abundance
#
# Gene: systematic gene name
# aATG.scorekw: annotated AUG score against wide -10 Kozak consensus sequence
# d1.scorekw: 1st downstream AUG score against wide -10 Kozak consensus sequence
# d1.posATG: position (in nt) of 1st downstream AUG compared to annotated AUG.
# d1.frame: downstream AUG is In or Out of reading frame with annotated.
#
# Edward Wallace, Apr 2019
",
file="data_out/dvsaATG_highdiffw_enoughRNA_JEC21.txt")
highdiffw_dvsaATG_enoughRNA_JEC21 %>%
    mutate_if(is.numeric,round3) %>%
    write_tsv("data_out/dvsaATG_highdiffw_enoughRNA_JEC21.txt",append = TRUE,col_names=TRUE)
```
...
ggplot(data=ribotbl_scoresenough_JEC21 %>%
    ...
    scale_y_log10(breaks=c(0.1,1,10),
                  limits=c(0.05,100),
                  oob=scales::squish) + 
    labs(x="Kozak narrow score, difference dATG - aATG",
         y="Occupancy ratio dATG / aATG") +
...
ggplot(data=dvsATGmito_JEC21,
    ...
    geom_diagline() +
    geom_point(size=0.5,colour="grey50") +
    # stat_smooth(method="lm",se=FALSE,fill="blue") + 
...
```{r uATG_TE_lm_JEC21, dependson="uATG_RPF_JEC21", results="show",fig.width=2.5,fig.height=3.5}

# ribo_uct_JEC21

lm( log10(TE) ~ uATGCtC,data=ribo_uct_JEC21) %>% summary

lm( log10(TE) ~ uATGCt, data=ribo_uct_JEC21) %>% summary

lm( log10(TE) ~ aATG.pos, data=ribo_uct_JEC21) %>% summary

lm( log10(TE) ~ uATGCtC + aATG.pos, data=ribo_uct_JEC21) %>% summary

lm( log10(TE) ~ uATGCt + aATG.pos, data=ribo_uct_JEC21) %>% summary
...
uATG_scoreTE_JEC21_data <- 
  ribo_uct_JEC21 %>%
  inner_join(scores_JEC21) %>%
  filter(uATGCt == 1 ) %>%
  left_join(ctbl_big_JEC21) %>%
  mutate(u1.posTSS20 =  cut(u1.posTSS,breaks=c(0,20,500)) %>%
           factor(levels=c("(0,20]","(20,500]"),
                  labels=c("uAUG ≤ 20nt","uAUG > 20nt"))
  )

summarystats_uATG_scoreTE_JEC21 <-
  uATG_scoreTE_JEC21_data %>%
  ungroup() %>%
  mutate(log10_TE=log10(TE)) %>%
  select(u1.posTSS20,u1.scorekw,log10_TE) %>%
  nest(-u1.posTSS20) %>%
  mutate(
    fit = map(data, ~ lm( log10_TE ~ u1.scorekw, data = .x) ),
    glanced = map(fit, glance)
  ) %>%
  select(-data,-fit) %>%
  unnest() %>%
  mutate(R=sqrt(r.squared)) %>%
  select(u1.posTSS20,R,r.squared,p.value) %>%
  mutate(R2label=paste("R^2 ==", signif(r.squared,2)),
         plabel =paste("p ==", signif(p.value,2))
  )
summarystats_uATG_scoreTE_JEC21
  
uATG_scoreTE_JEC21 <- 
ggplot(data=uATG_scoreTE_JEC21_data,
       aes(x=u1.scorekw,y=TE)) +
    facet_wrap(~u1.posTSS20) + 
    geom_point(colour="grey50",size=0.7) +
    stat_smooth(method="lm",fill="blue") + 
  geom_label(data=summarystats_uATG_scoreTE_JEC21,aes(label=R2label),
             x=0.6,y=log10(5.5),hjust=0,vjust=1,colour="blue",parse=TRUE,
             alpha=0.5) +
    scale_y_log10("TE of aORF",
                  limits=c(0.1,5),
                  breaks=c(0.1,0.2,0.5,1,2,5),
                  oob=scales::squish) +
    labs(x="uAUG Kozak score") +
    panel_border()
uATG_scoreTE_JEC21
```
...
```{r uATG_aATG_nocc_JEC21, dependson="calc_scores_JEC21", fig.width=4.5,fig.height=4}
ggplot(data=ribotbl_scoresenough_JEC21 %>%
    ...
    scale_y_log10(breaks=c(0.1,1,10),
                  ...
                  oob=scales::squish) + 
    facet_grid(Type ~ Sample) +
    labs(x="narrow score, difference u1AUG - aAUG",
         y="Occupancy ratio u1AUG / aAUG") +
    panel_border()
```
...
```{r uATG_aATGw_occ_JEC21, dependson="calc_scores_JEC21", fig.width=4.5,fig.height=4}
ggplot(data=ribotbl_scoresenough_JEC21 %>%
           mutate(Type=factor(Type,levels=c("RPF","RNA"))),
    ...
    scale_y_log10(breaks=c(0.1,1,10),
                  ...
                  oob=scales::squish) + 
    facet_grid(Type ~ Sample) +
    labs(x="wide score, difference u1AUG - aAUG",
         y="Occupancy ratio u1AUG / aAUG") +
    panel_border()
```
...
```{r uATG_aATGwsumm_occ_JEC21, dependson="calc_scores_JEC21",results="show", fig.width=4.5,fig.height=2.5}

ribotbl_UIratio_JEC21 <- 
  ribotbl_scoresenough_JEC21 %>%
  # mutate(Type=factor(Type,levels=c("RPF","RNA"))) %>%
  group_by(Sample,Type) %>%
  mutate(Upct.widenorm = Upct.wide / median(Inct.wide),
         Inct.widenorm = Inct.wide / median(Inct.wide),
         UIratio.widenorm = Upct.widenorm / Inct.widenorm) %>%
  ungroup() %>%
  group_by(Gene,Type,u1vsaw) %>%
  summarize(UIratio.widenorm = median(UIratio.widenorm,na.rm=TRUE),
            Upct.widenorm = median(Upct.widenorm,na.rm=TRUE),
            Inct.widenorm = median(Inct.widenorm,na.rm=TRUE) ) %>%
  filter( Gene %in% ( 
    uATG_ct_JEC21 %>%
      filter(uATGCt == 1 ) %>%
      .$Gene ) )

summarystats_UIratio_JEC21 <- 
  ribotbl_UIratio_JEC21 %>%
  ungroup() %>%
  filter(UIratio.widenorm > 0, is.finite(UIratio.widenorm)) %>%
  mutate(log10_UIratio.widenorm = log10(UIratio.widenorm) ) %>%
  select(Type,Gene,log10_UIratio.widenorm,u1vsaw) %>%
  nest(-Type) %>%
  mutate(
    fit = map(data, ~ lm( log10_UIratio.widenorm ~ u1vsaw, data = .x) ),
    glanced = map(fit, glance)
  ) %>%
  select(-data,-fit) %>%
  unnest() %>%
  mutate(R=sqrt(r.squared)) %>%
  select(Type,R,r.squared,p.value) %>%
  mutate(R2label=paste("R^2 ==", signif(r.squared,2)),
         plabel =paste("p ==", signif(p.value,2))
         )
...
uATG_aATGwsumm_occ_JEC21 <- 
    ggplot(data=ribotbl_UIratio_JEC21,
       aes(x=u1vsaw,y=UIratio.widenorm)) +
    geom_point(size=0.5,colour="grey50") +
    # stat_smooth(method="loess",se=TRUE,colour="blue",fill="blue",span=0.8) + 
    # stat_smooth(method="gam",formula=y~s(x,k=4),fill="blue") + 
    stat_smooth(method="lm",fill="blue") + 
    geom_label(data=summarystats_UIratio_JEC21,aes(label=R2label),
              x=-0.33,y=log10(15),hjust=0,vjust=0,colour="blue",parse=TRUE,
              alpha=0.5) +
    geom_label(data=summarystats_UIratio_JEC21,aes(label=plabel),
              x=-0.33,y=log10(15),hjust=0,vjust=1,colour="blue",parse=TRUE,
              alpha=0.5) +
    scale_x_continuous(expand=c(0.005,0.002),
    ...
    labs(x="Kozak score diff. uAUG - aAUG",
         y="Occupancy ratio\n uAUG : aAUG") +
    panel_border()
...
ctbl_big_H99 <- read_tsv("data_in/H99_10p_up12dwn9_allATGcontext.table.txt") %>% 
    ...
    dplyr::mutate_if(str_detect(colnames(.),pattern="context"),toupper) %>%
    select(keepcols_ctbl) %>%
    filter(Gene != "CNAG_03851") # this has a bad ATG annotation
ctbl_big_H99
```
...
```{r aATG_context_H99_svg,dependson="cytoribo_context",fig.width=3,fig.height=0.55,dev="svg"}
ggseqlogo(data=ctbl_big_H99$aATG.context,
                      namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
    annotate(geom="text",label="Cn all aATGs",
             x=2.5,y=2,size=4,vjust=1,hjust=0,family="Arial",fontface = "italic")
```
...
aAUG_hi_context_H99 <- 
    ...
    annotate(geom="segment",x=2.5,xend=15.5,y=-0.1,yend=-0.1,size=0.8,colour="grey50") +
    annotate(geom="text",label="+1",x=13,y=-0.2,hjust=0.5,vjust=1.2,colour="black") # +
    # annotate(geom="text",label="wide",x=9,y=-0.1,size=4, vjust=1.15,colour="grey50") 
aAUG_hi_context_H99

```

```{r a_hi_context_H99_svg,dependson=c("load_context_H99","load_riboviz_H99"),fig.width=4,fig.height=1,dev="svg"}
ggseqlogo(data=filter(ctbl_big_H99,Gene %in% hiTrans_H99)$aATG.context,
          namespace="TCAG") + 
  xlim(c(2.5,21.5)) + 
  theme_nothing() +
  annotate(geom="text",label="Cn top 5% aATGs",
           x=2.5,y=2,size=4,vjust=1,hjust=0,family="Arial",fontface = "italic")
```
...
```{r cytoribo_context_H99_svg,dependson="cytoribo_context",fig.width=3,fig.height=0.55,dev="svg"}
ggseqlogo(data=filter(ctbl_big_H99,Gene %in% cytoribolist_H99)$aATG.context,
              namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
    annotate(geom="text",label="C. neoformans",
             x=2.5,y=2,size=4,vjust=1,hjust=0,family="Arial",fontface = "italic")
```
...
```{r d1_context_H99_svg,dependson="cytoribo_context",fig.width=3,fig.height=0.55,dev="svg"}
ggseqlogo(data=filter(ctbl_big_H99,nchar(d1.context) ==24)$d1.context,
                      namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
    annotate(geom="text",label="Cn first dATGs",
             x=2.5,y=2,size=4,vjust=1,hjust=0,family="Arial",fontface = "italic")
```
...
scores_H99 <- 
    ...
    transmute(Gene,
              ...
              d2.scorekn   = PWMscoren(d2.context,kozak_n_PWM_H99),
              ...
              d2.scorekw   = PWMscorew(d2.context,kozak_w_PWM_H99),
...
plot_4ATGscoresw_dens(scores_H99,hiTrans_H99,xlab="Kozak score")
```

```{r plot_cumscores_w_H99, dependson="calc_scores_H99", dev="pdf",fig.width=5,fig.height=1.8}
plot_cumscores_w_H99 <- 
    plot_4ATGscoresw_ecdf(scores_H99,hiTrans_H99,xlab="Kozak score")
plot_cumscores_w_H99
```

### Wide consensus comparing d1ATG to d2AUG score

```{r plot_scores_wd1d2_H99, dependson="calc_scores_H99", fig.width=5,fig.height=1.8}
...
### Count unusual scores

```{r count_unusual_scores_w_H99, dependson="calc_scores_H99",results="show",fig.width=5,fig.height=1.8}
scores_H99 %>%
    ungroup() %>%
    summarize_if(str_detect(names(scores_H99),"scorekw"),median,na.rm=TRUE)
...
scores_H99 %>%
    count(dvsmeda=d1.scorekw>0.856,dvsmedhiTrans=d1.scorekw>0.926)
...
plot_ATGdiffs_dens(scores_H99,
                   xlab=expression(paste("Kozak score diff. ", d[1],"AUG - aAUG")),
                   difftype="downstream")

dvsaATG_cumscores_H99 <- 
    plot_ATGdiffs_ecdf(scores_H99,
                       xlab=expression(paste("Kozak score diff. ", d[1],"AUG - aAUG")),
                       difftype="downstream")
...
```{r dvsaATG_scoresw_H99, dependson="calc_scores_H99", fig.width=4.5,fig.height=3.5}
...
```{r highdiffw_dvsaATG_enoughRNA_H99,dependson="dvsaATG_scoresn_H99",results="show"}
highdiffw_dvsaATG_enoughRNA_H99 <- 
    highdiffw_dvsaATG_H99 %>%
    left_join( ctbl_big_H99 %>% select(Gene,d1.posATG,d1.frame) ) %>%
    filter(Gene %in% enoughRNA_H99) %>%
    mutate(d1.frame = (d1.frame == 0) %>% 
               factor(levels=c(TRUE,FALSE),labels=c("In","Out") ) ) %>%
    select(Gene,aATG.scorekw,d1.scorekw,d1.posATG,d1.frame)
...
cat(
    "# dvsaATG_highdiffw_enoughRNA_H99.txt
# Genes with in-frame high-score dATGs
# In C. neoformans H99:
#   - difference, aATG.scorekw - d1.scorekw > 0.1
#   - in top 50% by RNA abundance
...
# Edward Wallace, Apr 2019
",
file="data_out/dvsaATG_highdiffw_enoughRNA_H99.txt")
highdiffw_dvsaATG_enoughRNA_H99 %>%
    mutate_if(is.numeric,round3) %>%
    write_tsv("data_out/dvsaATG_highdiffw_enoughRNA_H99.txt",append = TRUE,col_names=TRUE)
```
...
```{r dATG_aATGn_occ_summ_H99, dependson="calc_scores_H99", fig.width=4,fig.height=2.5}
...
ggplot(data=ribotbl_scoresenough_H99 %>%
    ...
    geom_point(size=0.5,colour="grey50") +
    # stat_smooth(method="loess",se=TRUE,colour="blue",fill="blue",span=0.8) + 
    stat_smooth(method="gam",formula=y~s(x,k=4),colour="green3",fill="green3") + 
    scale_y_log10(breaks=c(0.1,1,10),
                  limits=c(0.05,50),
                  oob=scales::squish) + 
    labs(x="Kozak narrow score diff. d1AUG - aAUG",
         y=expression(atop("Occupancy ratio",paste(d[1],"AUG : aAUG")))) +
    facet_wrap(~Type,ncol=2) +
    panel_border()
...
```{r dATG_aATGw_occ_summ_H99, dependson="calc_scores_H99",results="show", fig.width=4,fig.height=2.5}

ribotbl_scoresenough_H99 %>%
  group_by(Gene,Type) %>%
  summarize(d1vsaScore = median(d1vsaw),
            d1vsaRibo = logmean(Nxct.wide / Inct.wide)) %>%
  mutate(d1vsaScut = cut(d1vsaScore,breaks=c(-1,0,1) ) ) %>%
  group_by(Type,d1vsaScut) %>%
  filter(d1vsaRibo > 0, is.finite(d1vsaRibo)) %>%
  summarize(R=cor(d1vsaScore,log10(d1vsaRibo),use="pairwise.complete.obs"))


dATG_aATGw_occ_summ_H99 <- 
ggplot(data=ribotbl_scoresenough_H99 %>%
           group_by(Gene,Type) %>%
           summarize(d1vsaScore = median(d1vsaw),
                     d1vsaRibo = logmean(Nxct.wide / Inct.wide))
    ...
    geom_point(size=0.5,colour="grey50") +
    # stat_smooth(method="loess",se=TRUE,colour="blue",fill="blue",span=0.8) + 
    stat_smooth(method="gam",formula=y~s(x,k=4),colour="blue",fill="blue") + 
    scale_y_log10(breaks=c(0.1,1,10),
                  limits=c(0.05,50),
                  oob=scales::squish) + 
    labs(x=expression(paste("Kozak score diff. ", d[1],"AUG - aAUG")),
         y=expression(atop("Occupancy ratio",paste(d[1],"AUG : aAUG")))) +
     facet_wrap(~Type,ncol=2) +
    panel_border()
dATG_aATGw_occ_summ_H99
...
```{r dvsaATG_scoresdiffpos_H99, dependson=c("calc_scores_H99","load_mitofates_H99"),fig.width=5,fig.height=2,results="show"}
# library(ggmosaic)
dvsabit_H99 <- scores_H99 %>% 
           filter(!is.na(d1.scorekn), !is.na(d1vsaw)) %>%
           left_join(ctbl_big_H99) %>% 
           mutate(d1vsawfac = cut(d1vsaw,
                                  breaks=c(-0.5,0.1,0.5),
...
dvsabit_H99 %>%  
    group_by(enoughRNA,d1vsawfac) %>%
    summarize(n=n(),
              d1pos.med=median(d1.posATG),d1pos.mean=mean(d1.posATG),
              d2pos.med=median(d2.posATG,na.rm=TRUE),d2pos.mean=mean(d2.posATG,na.rm=TRUE))

dvsabit_H99 %>%  
    group_by(enoughRNA,d1vsawfac,d1.framefac) %>%
    summarize(n=n(),
              d1pos.med=median(d1.posATG),d1pos.mean=mean(d1.posATG),
              d2pos.med=median(d2.posATG,na.rm=TRUE),d2pos.mean=mean(d2.posATG,na.rm=TRUE))

ggplot(data=dvsabit_H99,
       aes(x=d1.posATG,colour=d1vsawfac,linetype=enoughRNA)) +
    stat_ecdf() +
    themebits_ecdf +
    scale_x_continuous(limits=c(0,400),expand=c(0.01,0.01),oob=scales::squish)

# ggplot(data=dvsabit_H99,
#        aes(x=d2.posATG,colour=d1vsawfac,linetype=enoughRNA)) +
#     stat_ecdf() +
#     themebits_ecdf +
#     scale_x_continuous(limits=c(0,400),expand=c(0.01,0.01),oob=scales::squish)
```
...
```{r uATG_TE_lm_H99, dependson="uATG_RPF_H99", results="show",fig.width=2.5,fig.height=3.5}

# ribo_uct_H99

lm( log10(TE) ~ uATGCtC,data=ribo_uct_H99) %>% summary

lm( log10(TE) ~ uATGCt, data=ribo_uct_H99) %>% summary

lm( log10(TE) ~ aATG.pos, data=ribo_uct_H99) %>% summary

lm( log10(TE) ~ uATGCtC + aATG.pos, data=ribo_uct_H99) %>% summary

lm( log10(TE) ~ uATGCt + aATG.pos, data=ribo_uct_H99) %>% summary
...
uATG_scoreTE_H99 <- 
ggplot(data=uATG_scoreTE_H99_data,
       aes(x=u1.scorekw,y=TE)) +
    facet_wrap(~u1.posTSS20) + 
    geom_point(colour="grey50",size=0.7) +
    stat_smooth(method="lm",fill="blue") + 
  geom_label(data=summarystats_uATG_scoreTE_H99,aes(label=R2label),
             x=0.55,y=log10(5.5),hjust=0,vjust=1,colour="blue",parse=TRUE,
             alpha=0.5) +
    scale_y_log10("TE of aORF",
                  limits=c(0.1,5),
                  breaks=c(0.1,0.2,0.5,1,2,5),
...
ribo_uct_H99 %>%
    filter(Gene %in% c("CNAG_06246","CNAG_05574","CNAG_03140",
                       "CNAG_07813","CNAG_07695","CNAG_01709",
                       "CNAG_03578","CNAG_00784")
           ) %>%
    select(Gene,RNA,RPF,TE,uATGCt,uATGCtmin20) %>%
    left_join(ctbl_big_H99 %>% 
                  transmute(Gene,
...
ctbl_big_H99 %>%
    ...
    transmute(Gene,
              ...
              # d2.scorekn   = PWMscoren(d2.context,kozak_n_PWM_JEC21),
              # u1.scorekn   = PWMscoren(u1.context,kozak_n_PWM_JEC21),
              asw = PWMscorew(aATG.context,kozak_w_PWM_JEC21),
              # d1.scorekw   = PWMscorew(d1.context,kozak_w_PWM_JEC21),
              # d2.scorekw   = PWMscorew(d2.context,kozak_w_PWM_JEC21),
              u1sw   = PWMscorew(u1.context,kozak_w_PWM_JEC21),
              u2sw   = PWMscorew(u2.context,kozak_w_PWM_JEC21),
    )
    # ) %>%
    # select(Gene,u1sw=u1.scorekw,u2sw=u2.scorekw, asw=aATG.scorekw)
```
...
```{r uATG_aATG_nocc_H99, dependson="calc_scores_H99", fig.width=4.5,fig.height=4}
ggplot(data=ribotbl_scoresenough_H99 %>%
           filter(Sample %in% c("H99r1","H99r2")) %>%
           mutate(Type=factor(Type,levels=c("RPF","RNA"))),
       aes(x=u1vsan,y=Upct.wide / Inct.wide)) +
...
```{r uATG_aATGw_occ_H99, dependson="calc_scores_H99", fig.width=7,fig.height=4}
ggplot(data=ribotbl_scoresenough_H99 %>%
           mutate(Type=factor(Type,levels=c("RPF","RNA"))),
       aes(x=u1vsaw,y=Upct.wide / Inct.wide)) +
    geom_point(size=0.5,colour="grey50") +
...
```{r uATG_aATGwsumm_occ_H99, dependson="calc_scores_H99",results="show", fig.width=4.5,fig.height=2.5}
...
uATG_aATGwsumm_occ_H99 <- 
    ggplot(data=ribotbl_UIratio_H99,
       aes(x=u1vsaw,y=UIratio.widenorm)) +
    geom_point(size=0.5,colour="grey50") +
    # stat_smooth(method="loess",se=TRUE,colour="blue",fill="blue",span=0.8) + 
    # stat_smooth(method="gam",formula=y~s(x,k=4),fill="blue") + 
    stat_smooth(method="lm",fill="blue") + 
    geom_label(data=summarystats_UIratio_H99,aes(label=R2label),
              x=-0.39,y=log10(15),hjust=0,vjust=0,colour="blue",parse=TRUE,
              alpha=0.5) +
    geom_label(data=summarystats_UIratio_H99,aes(label=plabel),
              x=-0.39,y=log10(15),hjust=0,vjust=1,colour="blue",parse=TRUE,
              alpha=0.5) +
...
```{r dependson="H99_vs_JEC21",fig.width=3.8,fig.height=3.5}
...
ggplot(data=ribosum_H99,aes(x=RNA,y=RPF)) +
    ...
    scale_loglog(limits=c(1e-1,2e4),
                  ...
                  expand=c(0.01,0.01)) +
    labs(x="RNA abundance (TPM of aORF)",
         y="Ribosome occupancy\n(TPM of aORF)") +
    theme(legend.position="none") +
    panel_border()
plot_ribosum_TE_H99

set.seed(42)
plot_ribosum_TE_JEC21 <- 
ggplot(data=ribosum_JEC21,aes(x=RNA,y=RPF)) +
    geom_diagline() +
    geom_point(size=.5,colour="grey50") +
    geom_point(data=TE_unusual_cc,
...
```{r scores_cc,dependson=c("H99_vs_JEC21","dvsaATG_scoresw_H99","dvsaATG_scoresw_JEC21"),results="show"}
...
# conservation of high dATG vs aAUG score
highdiffw_dvsaATG_cc <- scores_cc  %>%
    filter(H99 %in% enoughRNA_H99,
           JEC21 %in% enoughRNA_JEC21,
           H99 %in% highdiffw_dvsaATG_H99$Gene,
           JEC21 %in% highdiffw_dvsaATG_JEC21$Gene) %>%
    arrange(aATG.scorekw.H99 - d1.scorekw.H99 +
                aATG.scorekw.JEC21 - d1.scorekw.JEC21) %>%
    inner_join(d1frame_cc %>% select(H99, d1.frame) ) %>%	
    select( H99, JEC21, d1.frame,
            a.skw.H99=aATG.scorekw.H99,
            d.skw.H99=d1.scorekw.H99,
            a.skw.JEC21=aATG.scorekw.JEC21,
            d.skw.JEC21=d1.scorekw.JEC21) 
# I had to split this instead of piping through with %>%
# because otherwise I was getting an error with object aATG.scorekw.H99 not found
# This makes no sense to me (EW).
# highdiffw_dvsaATG_cc <- 
#     highdiffw_dvsaATG_cc %>%	
#     select( H99, JEC21, d1.frame,
#             a.skw.H99=aATG.scorekw.H99,
#             d.skw.H99=d1.scorekw.H99,
...
```{r dvsaATG_highdiffw_inframe_cc,dependson="scores_cc",results="show"}
...
cat(
    "# dvsaATG_highdiffw_inframe_cc.txt
# Genes with conserved in-frame high-score dATGs
# In both H99 and JEC21:
#   - aATG.scorekw - d1.scorekw < 0.1 (big difference)
#   - in top 50% by RNA abundance
#   - dATG is IN frame with aATG
#
# H99: H99 systematic gene name
# JEC21: JEC21 systematic gene name of ortholog
# a.skw.xx: annotated AUG score against wide -10 Kozak consensus sequence, strain xx
# d.skw.xx: 1st downstream AUG score against wide -10 Kozak consensus sequence, strain xx
#
# Edward Wallace, Apr 2019
",
file="data_out/dvsaATG_highdiffw_inframe_cc.txt")
highdiffw_dvsaATG_cc %>%
    filter(d1.frame == 0) %>%
    select(-d1.frame) %>%
    mutate_if(is.numeric,round3) %>%
    write_tsv("data_out/dvsaATG_highdiffw_inframe_cc.txt",append = TRUE,col_names=TRUE)
```
...
```{r dvsaATG_highdiffw_outframe_cc,dependson="scores_cc",results="show"}
# print top hits in frame
highdiffw_dvsaATG_cc %>%
    filter(d1.frame != 0) %>%
    select(-d1.frame) %>%
...
cat(
    "# dvsaATG_highdiffw_outframe_cc.txt
...
# Edward Wallace, Apr 2019
",
file="data_out/dvsaATG_highdiffw_outframe_cc.txt")
highdiffw_dvsaATG_cc %>%
    filter(d1.frame != 0) %>%
    select(-d1.frame) %>%
    mutate_if(is.numeric,round3) %>%
    write_tsv("data_out/dvsaATG_highdiffw_outframe_cc.txt",append = TRUE,col_names=TRUE)
```
...
# scores_cc
highdiffw_aATG_HvsJ_cc <- scores_cc  %>%
    mutate(askwdiff.HvsJ=aATG.scorekw.H99-aATG.scorekw.JEC21) %>%
    filter(H99 %in% enoughRNA_H99,
           JEC21 %in% enoughRNA_JEC21,
           abs(askwdiff.HvsJ) >= 0.1) 

highdiffw_aATG_HvsJ_cc_nice <- highdiffw_aATG_HvsJ_cc  %>%	
    transmute( H99, JEC21, # d1.frame,
            a.skw.H99=round(aATG.scorekw.H99,5),
            d.skw.H99=round(d1.scorekw.H99,5),
            a.skw.JEC21=round(aATG.scorekw.JEC21,5),
            d.skw.JEC21=round(d1.scorekw.JEC21,5),
            askwdiff.HvsJ) 

highdiffw_aATG_HvsJ_cc_nice %>%
    arrange(askwdiff.HvsJ) %>%
    select(-askwdiff.HvsJ)

highdiffw_aATG_HvsJ_cc_nice %>%
    arrange(desc(askwdiff.HvsJ))  %>%
    select(-askwdiff.HvsJ)

ggplot(data=scores_cc,aes(x=aATG.scorekw.H99,y=aATG.scorekw.JEC21)) +
    geom_diagline() +
    geom_point()
```
...
```{r utRS_seq,dependson="scores_cc",fig.width=4,fig.height=1,results="show"}
unusual_tRNAsynth_JEC21 <-
    c("CNI03160","CNB05640","CNB00480","CNF02520","CNF00730",
      "CNJ00430","CNB01880","CNC06400","CNB00900")
utRS_contexts_cc <- 
    ctbl_cc  %>%
    filter(JEC21 %in% unusual_tRNAsynth_JEC21) %>%
    transmute(H99,JEC21,
              aATG.context.H99 = str_sub(aATG.context.H99,start=3L,end=15L),
              aATG.context.JEC21 = str_sub(aATG.context.JEC21,start=3L,end=15L),
              d1.context.H99 = str_sub(d1.context.H99,start=3L,end=15L),
              d1.context.JEC21 = str_sub(d1.context.JEC21,start=3L,end=15L)) 

utRS_contexts_cc

cat(
    "# dualAUG_aaRS_contexts_cc.txt
# Sequence contexts aATGs of 9 dualAUG tRNA synthetases
# This is of the wide ATG context NNNNNNNNNNATG
# In both H99 and JEC21
#
# Edward Wallace, Apr 2019
",
file="data_out/dualAUG_aaRS_contexts_cc.txt")
utRS_contexts_cc %>%
    write_tsv("data_out/dualAUG_aaRS_contexts_cc.txt",
              append = TRUE,col_names=TRUE)
```

```{r utRS_motif,dependson=c("utRS_seq","scores_cc"),fig.width=4,fig.height=1,results="show"}
ggseqlogo(data=ctbl_big_JEC21 %>%
...
scores_cc %>%
    select(H99,JEC21,
           aATG.scorekw.H99,d1.scorekw.H99,u1.scorekw.H99,
           aATG.scorekw.JEC21,d1.scorekw.JEC21,u1.scorekw.JEC21
           ) %>%
...
# H99: H99 systematic gene name
# JEC21: JEC21 systematic gene name of ortholog
# aATG.scorekw.xx: score of annotated ATG against wide Kozak sequence (NNNNNNNNNNATG), strain xx
# d1.scorekw.xx: score of 1st downstream ATG against wide Kozak sequence, strain xx
# u1.scorekw.xx: score of 1st upstream ATG in 5' UTR against wide Kozak sequence, strain xx
# aATG.scoreUw.xx: score of annotated ATG against wide Unusual tRNA Synthetase sequence (NNNNATG), strain xx
# aATG.scoreUw.xx: score of annotated ATG against wide Unusual tRNA Synthetase sequence (NNNNATG), strain xx
...
highdiffw_dvsaATG_cc %>%
    filter(d1.frame == 0) %>%
    select(Gene=JEC21,
           a.scorekw=a.skw.JEC21,d.scorekw=d.skw.JEC21) %>%
    left_join(ctbl_big_JEC21 %>% 
                  transmute(Gene,
                            a.scoreUw=PWMscorew(aATG.context,
                                                pwm=utRS_w_PWM),
                            aATG.context) 
              ) %>%
    left_join(mitofates_JEC21 %>% select(Gene,Prob_preseq)) %>%
    filter(a.scoreUw > 0.9)
...
ggplot(data=scores_kvsu_JEC21 %>% 
    ...
    geom_density(kernel="rectangular",bw=0.025) +
        scale_colour_manual(values=c("Yes"="magenta","No"="grey50"),
                        labels=mitobreaks_JEC21)  +
    labs(x="difference in wide aAUG score, utRS vs Kozak", colour="Mito. pred") +
    theme(axis.line.y=element_blank(),axis.ticks.y=element_blank(),
          axis.text.y=element_blank(),axis.title.y=element_blank())
```
...
highdiffw_dvsaATG_cc %>% filter(JEC21 %in% unusual_tRNAsynth_JEC21)
scores_JEC21 %>% 
    filter(Gene %in% unusual_tRNAsynth_JEC21) %>% 
    select(Gene,aATG.scorekn,d1.scorekn) %>%
...
```{r fig2top_TE_uAUG_H99,dependson=c("uATG_TE_H99","uATGmin20_TE_H99","H99_vs_JEC21"),fig.width=7.5,fig.height=3.4}
plot_grid(plot_ribosum_TE_H99,uATG_TE_H99,uATGmin20_TE_H99,
          ncol=3,rel_widths = c(3.7,1.9,1.9),labels=c("A","E","F"),align="h")
```

### Supp fig2: TE and uAUG usage in JEC21


```{r fig2top_TE_uAUG_JEC21,dependson=c("uATG_TE_JEC21","uATGmin20_TE_JEC21","JEC21_vs_JEC21"),fig.width=7.5,fig.height=3.4}
plot_grid(plot_ribosum_TE_JEC21,uATG_TE_JEC21,uATGmin20_TE_JEC21,
          ncol=3,rel_widths = c(3.7,1.9,1.9),labels=c("A","E","F"),align="h")
```

### Figure 3 panels: score and AUG/uAUG usage in H99

```{r fig3_AUGscore_H99,dependson=c("a_hi_context_H99","plot_cumscores_w_H99","uvsaATG_scoresdens_H99","uATG_scoreTE_H99","uATG_aATGwsumm_occ_H99"),fig.width=7.5,fig.height=4.5}
options(scipen=-1)
left_col <- 
    plot_grid(aAUG_hi_context_H99,plot_cumscores_w_H99,uvsaATG_cumscores_H99,
          ncol=1,rel_heights = c(1,2,2),
          labels=c("A","B","C"),label_y=c(1,1.05,1.05),align="v")

right_col <- 
    plot_grid(uATG_scoreTE_H99,
              uATG_aATGwsumm_occ_H99,
          ncol=1,labels=c("D","E"),align="v")

plot_grid(left_col,right_col,ncol=2,rel_widths = c(1.2,1))
```

### Figure S3 panels: score and AUG/uAUG usage in JEC21


```{r fig3_AUGscore_JEC21,dependson=c("a_hi_context_JEC21","plot_cumscores_w_JEC21","uvsaATG_scoresdens_JEC21","uATG_scoreTE_JEC21","uATG_aATGwsumm_occ_JEC21"),fig.width=7.5,fig.height=4.5}
options(scipen=-1)
left_col <- 
    plot_grid(aAUG_hi_context_JEC21,plot_cumscores_w_JEC21,uvsaATG_cumscores_JEC21,
          ncol=1,rel_heights = c(1,2,2),
          labels=c("A","B","C"),label_y=c(1,1.05,1.05),align="v")

right_col <- 
    plot_grid(uATG_scoreTE_JEC21,
              uATG_aATGwsumm_occ_JEC21,
          ncol=1,labels=c("D","E"),align="v")
...
```{r fig6top_dAUGscore_H99,dependson=c("dvsaATG_scoresw_H99","dATG_aATGw_occ_summ_H99"),fig.width=7.5,fig.height=2.5}

plot_grid(dvsaATG_scoresw_H99,dATG_aATGw_occ_summ_H99,
          ncol=2,rel_heights = c(2.8,4.7),
