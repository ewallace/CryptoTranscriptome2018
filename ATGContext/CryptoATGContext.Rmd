...
ggplot(data=ctbl_big_JEC21 %>%
    ...
    geom_density(aes(x=d1.scorekn,colour=d1.frame),
                 kernel="rectangular",bw=sbandwidthn) +
    labs(x="d1ATG Kozak score, JEC21") +
    theme(axis.line.y=element_blank(),axis.ticks.y=element_blank(),
          axis.text.y=element_blank(),axis.title.y=element_blank())
```
...
```{r plot_scores_nd1d2_mitofates_JEC21, dependson=c("calc_scores_JEC21","load_mitofates_JEC21"), fig.width=5,fig.height=1.8}
...
```{r plot_scores2_nd1d2_mitofates_JEC21, dependson=c("calc_scores_JEC21","load_mitofates_JEC21"), fig.width=5,fig.height=1.8,results="show",eval=FALSE}
...
melt_mito_scores_JEC21 <- 
    ...
    select(Gene,Pred_preseq,
           aATG=aATG.scorekn,dATG1=d1.scorekn,dATG2=d2.scorekn) %>%
    gather(key="ATG",value="scorekn",aATG,dATG1,dATG2)
...
ggplot(data=melt_mito_scores_JEC21,
       aes(x=ATG,linetype=Pred_preseq,colour=ATG,y=scorekn)) +
...
make_uATGfact <- function(Ct,Ctfar) {
    sapply(1:length(Ct), function(i) .make_uATGfact(Ct[i],Ctfar[i])) %>%
        factor(levels=c("0","1c","1f","2+c","2+f"),
               labels=c("0","1\nclose","1\nfar","2+\nclose","2+\nfar"))
}
...
```{r uATGmin20_TE_JEC21, dependson="uATG_RPF_JEC21", fig.width=3.5,fig.height=3.5,results="show"}
# ribo_uct_JEC21
ggplot(data=ribo_uct_JEC21,
       aes(x=uATGfac,y=TE)) +
    geom_jitter(colour="grey50",size=0.7) +
...
- CNAG_04443, hydrolase homolog. Duplicate orthologs in pombe and nidulans suggest dual-localization. H99 CNAG_04443 has ribosomes only from d1ATG, MJEC21 CNI01140 has ribosomes on uORF.
...
- CNAG_07352, mito complex MICOS MIC26 homolog. JEC21 CNA03190, short 5'UTR, initiation downstream of annotated ATG, also alt 5'UTR intron.
