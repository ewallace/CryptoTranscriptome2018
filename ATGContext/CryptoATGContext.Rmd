...
```{r kozak_inf_JEC21,fig.width=4,fig.height=1}
...
entropytot(kozak_w_PFM_JEC21)
# entropytot(kozak_w_PFM_JEC21[,5:12])
entropytot(kozak_w_PFM_JEC21[,-(10:12)])
entropytot(kozak_w_PFM_JEC21[,5:9])
```

Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r entropytot(kozak_w_PFM_JEC21[,5:9]) %>% round(2)`, of **wide** is `r entropytot(kozak_w_PFM_JEC21[,-(10:12)]) %>% round(2)`.
...
```{r uATG_RPF_JEC21, dependson="calc_scores_JEC21", fig.width=3.5,fig.height=3.5}
uATG_ct_JEC21 <- read_tsv("data_in/JEC21_10p_up12dwn9_allATGcontext.table.txt") %>% 
    dplyr::rename(Gene=gene) %>%
    dplyr::select(c("Gene",paste0("u",1:56,".posATG"))) %>%
    gather(key="uATG",value="posaATG",-Gene) %>%
    group_by(Gene) %>%
    summarize(uATGCt = sum(!is.na(posaATG))) %>%
    mutate(uATGCtC = replace(uATGCt,uATGCt>=4, "4+") %>%
               factor(levels=c(0:3,"4+")) )
...
ggplot(data=ribo_uct_JEC21,
    ...
    scale_y_log10nice("Ribosome occupancy (TPM of aORF)",
                      limits=c(1,10000),
                      oob=scales::squish) +
    labs(x="number of uATGs")
```

### uATGs are associated with lower translation efficiency

```{r uATG_TE_JEC21, dependson="uATG_RPF_JEC21", fig.width=3.5,fig.height=3.5}
...
```{r kozak_inf_H99,fig.width=4,fig.height=1}
entropy1 <- function(counts,ssize=4,pseudocount=0.5) {
    counts <- counts + pseudocount
    ntot = sum(counts)
    freqs = counts / sum(counts)
...
entropytot(kozak_w_PFM_H99)
# entropytot(kozak_w_PFM_H99[,5:12])
entropytot(kozak_w_PFM_H99[,-(10:12)])
entropytot(kozak_w_PFM_H99[,5:9])
```

Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r entropytot(kozak_w_PFM_H99[,5:9]) %>% round(2)`, of **wide** is `r entropytot(kozak_w_PFM_H99[,-(10:12)]) %>% round(2)`.
