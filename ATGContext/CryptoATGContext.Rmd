...
kozak_w_PFM_JEC21 <- filter(ctbl_big_JEC21,Gene %in% hiTrans_JEC21) %>%
    .$aATG.context %>%
    str_sub(start=3L,end=15L) %>%
    consensusMatrix() 

kozak_w_PWM_JEC21 <- PWM(kozak_w_PFM_JEC21)

kozak_n_PFM_JEC21 <- filter(ctbl_big_JEC21,Gene %in% hiTrans_JEC21) %>%
    .$aATG.context %>%
    str_sub(start=9L,end=15L) %>%
    consensusMatrix() 
kozak_n_PWM_JEC21 <- PWM(kozak_n_PFM_JEC21)
```
...
Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r infontot(kozak_n_PFM_JEC21[,-(5:7)]) %>% round(2)`, of **wide** is `r infontot(kozak_w_PFM_JEC21[,-(10:12)]) %>% round(2)`.
...
ggplot(data=scores_JEC21 %>% 
    ...
    scale_colour_manual(values=c("Yes"="magenta","No"="grey50"),
                        labels=mitobreaks_JEC21)  + 
    labs(x="narrow score, difference dATG - aATG", colour="Mito. pred") +
    theme(axis.line.y=element_blank(),axis.ticks.y=element_blank(),
          axis.text.y=element_blank(),axis.title.y=element_blank())
```
...
kozak_n_PFM_H99 <- filter(ctbl_big_H99,Gene %in% hiTrans_H99) %>%
    .$aATG.context %>%
    str_sub(start=9L,end=15L) %>%
    consensusMatrix() 
kozak_n_PWM_H99 <- PWM(kozak_n_PFM_H99)
```
...
Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r infontot(kozak_n_PFM_H99[,-(5:7)]) %>% round(2)`, of **wide** is `r infontot(kozak_w_PFM_H99[,-(10:12)]) %>% round(2)`.
...
scores_kvsu_H99 <-
    ...
    left_join(ctbl_big_H99 %>% 
                  transmute(Gene,
                            aATG.scoreun=PWMscoren(aATG.context,
                                                pwm=utRS_n_PWM),
                            aATG.scoreuw=PWMscoren(aATG.context,
                                                pwm=utRS_w_PWM),
                  ) )
