...
aAUG_hi_context_JEC21 <- aAUG_hi_context_JEC21_bare  +
    ...
    annotate(geom="segment",x=3.5,xend=15.5,y=-0.65,yend=-0.65,size=1,colour="grey50") +
    annotate(geom="text",label="wide",x=9,y=-0.65,size=4, vjust=1.15,colour="grey50") 
```
...
plot_cumscores_w_JEC21 <- 
    plot_4ATGscoresw_ecdf(scores_JEC21,hiTrans_JEC21,xlab="wide Kozak score, JEC21")
plot_cumscores_w_JEC21
...
plot_ATGdiffs_dens(scores_JEC21,
                   xlab="Kozak score diff. d1AUG - aAUG",
                   difftype="downstream")
...
plot_ATGdiffs_dens(scores_JEC21,
                   xlab="Kozak score diff. u1AUG - aAUG",
                   difftype="upstream")
...
ggplot(data=scores_JEC21 %>% 
    ...
    coord_equal() + 
    labs(x="aAUG wide Kozak score",
         y="d1AUG wide Kozak score")
```
...
# aATG.scorekn: annotated AUG score against narrow -4 Kozak consensus sequence
# d1.scorekn: 1st downstream AUG score against narrow -4 Kozak consensus sequence
#
...
ggplot(data=ribotbl_scoresenough_JEC21 %>%
    ...
    labs(x="narrow score, difference u1AUG - aAUG",
         y="Ribosome occupancy, ratio u1AUG / aAUG") +
    panel_border()
```
...
uATG_aATGwsumm_occ_JEC21 <- 
    ...
    labs(x="wide score diff. u1AUG - aAUG",
         y="occupancy ratio \n u1AUG : aAUG") +
    panel_border()
...
plot_4ATGscoresw_dens(scores_H99,hiTrans_H99,xlab="wide Kozak score, H99")

```

```{r plot_cumscores_w_H99, dependson="calc_scores_H99", fig.width=5,fig.height=1.8}
plot_cumscores_w_H99 <- 
    plot_4ATGscoresw_ecdf(scores_H99,hiTrans_H99,xlab="wide Kozak score, H99")
plot_cumscores_w_H99
...
```{r dvsaATG_scoresw_plain_H99, dependson="calc_scores_H99", fig.width=3.5,fig.height=3.5}
highdiffw_dvsaATG_H99 <- scores_H99 %>%
    arrange(desc(d1vsaw)) %>%
    head(n=330) %>%
    select(Gene,aATG.scorekw,d1.scorekw)
...
ggplot(data=ribotbl_scoresenough_H99 %>%
    ...
    scale_y_log10(breaks=c(0.1,1,10),
                  limits=c(0.05,50),
                  oob=scales::squish) + 
    labs(x="Kozak wide score diff. d1AUG - aAUG",
         y="Ribosome occupancy\nratio d1AUG : aAUG") +
     facet_wrap(~Type,ncol=2) +
    panel_border()
dATG_aATGw_occ_summ_H99
...
# H99: H99 systematic gene name
# JEC21: JEC21 systematic gene name of ortholog
# a.skn.xx: annotated AUG score against narrow -4 Kozak consensus sequence, strain xx
# d.skn.xx: 1st downstream AUG score against narrow -4 Kozak consensus sequence, strain xx
#
# Edward Wallace, Jan 2019
",
...
plot_grid(plot_ribosum_TE_H99,uATG_TE_H99,uATGmin20_TE_H99,
          ncol=3,rel_widths = c(3.7,1.9,1.9),labels=c("A","B","C"),align="h")
```
