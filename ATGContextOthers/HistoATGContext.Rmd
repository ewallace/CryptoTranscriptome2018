...
kozak_all_PFM_Hc <- filter(ctbl_big_Hc,Gene %in% hiTrans_Hc) %>%
    .$aATG.context %>%
    
    str_to_upper() %>%
    consensusMatrix() 

kozak_w_PWM_Hc <- PWM(kozak_all_PFM_Hc[,3:15])
kozak_n_PWM_Hc <- PWM(kozak_all_PFM_Hc[,9:15])
```
...
Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r infontot(kozak_all_PFM_Hc[,5:9]) %>% round(2)`, of **wide** is `r infontot(kozak_all_PFM_Hc[,-(10:12)]) %>% round(2)`.
