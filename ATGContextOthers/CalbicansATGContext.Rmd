...
nonterribleTrans_Ca <- ribosum_Ca %>%
    arrange(desc(RPF)) %>%
    head(n=4500) %>%
    .$Gene
...
ctbl_big_Ca <- read_ATGcontext_table("data_in/CAlbi_allATGcontext.table.txt")
ctbl_big_Ca
```
...
```{r cytoribo_context_Ca,dependson="load_context_Ca",fig.width=4,fig.height=1}
cytoribolist_Ca <- read_tsv("data_out/ComplexOrthologs/cytoribosome_PTRcurated6.txt",
                        comment="#") %>%
    filter(SPECIES=="CANAL") %>%
    .$GENE_ID
ggseqlogo(data=filter(ctbl_big_Ca,Gene %in% cytoribolist_Ca)$aATG.context,
                      namespace="TCAG") + 
    theme_nothing()
```
...
kozak_all_PFM_Ca <- filter(ctbl_big_Ca,Gene %in% hiTrans_Ca) %>%
    .$aATG.context %>%
    str_to_upper() %>%
    consensusMatrix() 

kozak_w_PWM_Ca <- PWM(kozak_all_PFM_Ca[,3:15])
kozak_n_PWM_Ca <- PWM(kozak_all_PFM_Ca[,9:15])
```
...
cat(
    "# PFM_hiTrans_Ca.txt
# Position Frequency Matrix for aATGs of 400 highest-translated genes from Ca
...
```{r kozak_inf_Ca,dependson=c("kozak_consensus_Ca","cytoribo_context_Ca"),fig.width=4,fig.height=1,results="show"}
inf_content_narrow <- 
    tribble(
        ...
        ctbl_big_Ca %>%
            filter(Gene %in% cytoribolist_Ca) %>%
            .$aATG.context %>%
            infonfromseqs(sstart=9L,send=12L),
        "All", "d1ATG", "narrow",
        ctbl_big_Ca %>%
...
Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r infontot(kozak_all_PFM_Ca[,5:9]) %>% round(2)`, of **wide** is `r infontot(kozak_all_PFM_Ca[,-(10:12)]) %>% round(2)`.


### Estimate information content per base across start context

```{r kozak_inf_perbase_Ca,dependson=c("kozak_consensus_Ca","cytoribo_context_Ca"),fig.width=6,fig.height=1.4,results="show"}
inf_content_perbase_Ca <- 
    tibble(i = 1:24, 
           Pos = c(-12:-1, 1:12),
        All = ctbl_big_Ca %>%
            .$aATG.context %>%
        ...
        HiTrans = ctbl_big_Ca %>%
            ...
            .$aATG.context %>%
            infonfromseqs(scope="all"),
        CytoRibo = ctbl_big_Ca %>%
            filter(Gene %in% cytoribolist_Ca) %>% 
            .$aATG.context %>%
            infonfromseqs(scope="all")
    )

write_tsv(inf_content_perbase_Ca %>%
              mutate_if(is.numeric,round3),
          "data_out/inf_content_perbase_Ca.txt")
# inf_content_perbase

plot_inf_perbase(inf_content_perbase_Ca)
```
...
scores_Ca <- 
    ctbl_big_Ca %>%
    transmute(Gene,
              aATG.scorekn = PWMscoren(str_to_upper(aATG.context),kozak_n_PWM_Ca),
              d1.scorekn   = PWMscoren(d1.context,kozak_n_PWM_Ca),
              # u1.scorekn   = PWMscoren(u1.context,,kozak_n_PWM_Ca)
              NULL
    ) %>%
...
```{r mutinf_Ca,dependson=c("load_context_Ca","load_riboviz_Ca"),results="show",fig.height=4,fig.width=5}
mutinf_Ca <- mutinfostr(ctbl_big_Ca$aATG.context)
# mutinf_Ca

plot_mutinfo(mutinf_Ca)
```

### For highly translated genes

```{r mutinf_hiTrans_Ca,dependson=c("load_context_Ca","load_riboviz_Ca"),results="show",fig.height=4,fig.width=5}
ctbl_big_Ca %>%
    filter(Gene %in% hiTrans_Ca) %>%
    .$aATG.context %>%
    mutinfostr %>%
...
```{r mutinf_cytoribo_Ca,dependson=c("load_context_Ca","load_riboviz_Ca"),results="show",fig.height=4,fig.width=5}
ctbl_big_Ca %>%
    filter(Gene %in% cytoribolist_Ca) %>%
    .$aATG.context %>%
    mutinfostr %>%
...
```{r cytoribo_contextm2_Ca,dependson="load_context_Ca",fig.width=4,fig.height=3}

seqlogoFilterGenePos <- function(datain=ctbl_big_Ca,Genelist=hiTrans_Ca,
                                 pos=8L, nt="T") {
...
plot_grid(seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=11L,nt="T"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=11L,nt="C"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=11L,nt="A"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=11L,nt="G"),
    ncol=1
)
...
```{r cytoribo_contextm4_Ca,dependson="load_context_Ca",fig.width=4,fig.height=3}
...
plot_grid(seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=9L,nt="T"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=9L,nt="C"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=9L,nt="A"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Ca,pos=9L,nt="G"),
    ncol=1
)
...
Saved to *dvsaATG_highdiffn_inframe_Ca.txt*.

```{r highdiffn_dvsaATG_inframe_Ca,dependson="dvsaATG_scoresn_Ca",results="show"}
highdiffn_dvsaATG_enoughinframe_Ca <- 
    highdiffn_dvsaATG_Ca %>%
    left_join( ctbl_big_Ca %>% select(Gene,d1.frame) ) %>%
    filter(Gene %in% nonterribleTrans_Ca, d1.frame==0) %>%
    select(Gene,aATG.scorekn,d1.scorekn)
...
```{r highdiffn_dvsaATG_outframe_Ca,dependson="dvsaATG_scoresn_Ca",results="show"}
highdiffn_dvsaATG_enoughoutframe_Ca <- 
    highdiffn_dvsaATG_Ca %>%
    left_join( ctbl_big_Ca %>% select(Gene,d1.frame) ) %>%
    filter(Gene %in% nonterribleTrans_Ca, d1.frame!=0) %>%
    select(Gene,aATG.scorekn,d1.scorekn)
...
```{r load_mitofates_Ca}
mitofates_Ca <- read_mitofates("data_in/Calbi_mitofates.txt")
