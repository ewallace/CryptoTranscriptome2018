...
tpms <- read_tsv("data_in/ExpressionTranslation/Calbicans_Muzzey2015_TPMs_collated.tsv") %>%
    dplyr::rename(Gene=ORF) %>%
    mutate(RPF = (WTA1 + WTA2 + 2 * WTB) / 4, 
           RNA = ( WTARNA + WTBRNA ) / 2 )
...
ggseqlogo(data=filter(ctbl_big,Gene %in% hiTrans)$aATG.context,
    ...
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
    annotate(geom="text",label="C. albicans",
...
ggplot(data=scores) +
    ...
    scale_colour_manual(values=c("aATG"="dodgerblue",
                                 ...
                                 # "uATG"="red",
                                 "dATG"="green3") ) +
    labs(x="ATG narrow Kozak score, Ca") +
...
ggplot(data=scores) +
    ...
    scale_colour_manual(values=c("aATG"="dodgerblue",
                                 ...
                                 "uATG"="red",
                                 "dATG"="green3") ) +
    labs(x="ATG wide Kozak score, Ca") +
    theme(axis.line.y=element_blank(),axis.ticks.y=element_blank(),
          axis.text.y=element_blank(),axis.title.y=element_blank(),
          legend.title=element_blank())
```
...
### Genelist with unusual dATG vs aATG narrow score
...
Files with high difference in narrow score, filtered for top 50% by mRNA abundance, in frame.
...
Files with high difference in narrow score, filtered for top 50% by mRNA abundance, out of frame. 
...
Files with high difference in wide score, filtered for top 50% by mRNA abundance, in frame.

Saved to *dvsaATG_highdiffw_inframe_Ca.txt*.

```{r highdiffw_dvsaATG_inframe,dependson="dvsaATG_scoresw",results="show"}
...
cat(
    "# dvsaATG_highdiffw_inframe_Ca.txt
# Genes with in-frame high-score dATGs
# In C. albicans SC5314:
...
#
# Gene: systematic gene name
# aATG.scorekw: annotated ATG score against narrow Kozak consensus sequence, strain xx
# d1.scorekw: 1st downstream ATG score against narrow Kozak consensus sequence, strain xx
#
# Edward Wallace, Jan 2019
",
file="data_out/dvsaATG_highdiffw_inframe_Ca.txt")
highdiffw_dvsaATG_enoughinframe %>%
    mutate_if(is.numeric,round3) %>%
    write_tsv("data_out/dvsaATG_highdiffw_inframe_Ca.txt",append = TRUE,col_names=TRUE)
```


### dATG out of frame with ATG, wide

Files with high difference in wide score, filtered for top 50% by mRNA abundance, out of frame. 

Saved to *dvsaATG_highdiffw_outframe_Ca.txt*.

```{r highdiffw_dvsaATG_outframe,dependson="dvsaATG_scoresw",results="show"}
...
cat(
    "# dvsaATG_highdiffw_outframe_Ca.txt
# Genes with in-frame high-score dATGs
# In C. albicans SC5314:
#   - in top 5% by difference, aATG.scorekw - d1.scorekw
#   - in top 50% by mRNA abundance
...
# Edward Wallace, Jan 2019
",
file="data_out/dvsaATG_highdiffw_outframe_Ca.txt")
highdiffw_dvsaATG_enoughoutframe %>%
    mutate_if(is.numeric,round3) %>%
    write_tsv("data_out/dvsaATG_highdiffw_outframe_Ca.txt",append = TRUE,col_names=TRUE)
```
...
write_tsv(dvsaATG_framemitosumm,
          "data_out/dvsaATG_framemitosumm_Ca.txt")
```
...
Several of these are conserved (ALA1, VAS1, PUS7, GLOs, GLR)!
