...
kozak_all_PFM_Sp <- filter(ctbl_big_Sp,Gene %in% hiTrans_Sp) %>%
    .$aATG.context %>%
    
    str_to_upper() %>%
    consensusMatrix() 

kozak_w_PWM_Sp <- PWM(kozak_all_PFM_Sp[,3:15])
kozak_n_PWM_Sp <- PWM(kozak_all_PFM_Sp[,9:15])
```
...
Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r infontot(kozak_all_PFM_Sp[,5:9]) %>% round(2)`, of **wide** is `r infontot(kozak_all_PFM_Sp[,-(10:12)]) %>% round(2)`.


### Estimate information content per base across start context

```{r kozak_inf_perbase_Sp,dependson=c("kozak_consensus_Sp","cytoribo_context_Sp"),fig.width=6,fig.height=1.4,results="show"}
inf_content_perbase_Sp <- 
    tibble(i = 1:24, 
           Pos = c(-12:-1, 1:12),
        All = ctbl_big_Sp %>%
            .$aATG.context %>%
        ...
        HiTrans = ctbl_big_Sp %>%
            ...
            .$aATG.context %>%
            infonfromseqs(scope="all"),
        CytoRibo = ctbl_big_Sp %>%
            filter(Gene %in% cytoribolist_Sp) %>% 
            .$aATG.context %>%
            infonfromseqs(scope="all")
    )

write_tsv(inf_content_perbase_Sp %>%
              mutate_if(is.numeric,round3),
          "data_out/inf_content_perbase_Sp.txt")
# inf_content_perbase

plot_inf_perbase(inf_content_perbase_Sp)
```
...
```{r mutinf_Sp,dependson=c("load_context_Sp","load_riboviz_Sp"),results="show",fig.height=4,fig.width=5}
mutinf_Sp <- mutinfostr(ctbl_big_Sp$aATG.context)
# mutinf_Sp

plot_mutinfo(mutinf_Sp)
```

### For highly translated genes

```{r mutinf_hiTrans_Sp,dependson=c("load_context_Sp","load_riboviz_Sp"),results="show",fig.height=4,fig.width=5}
ctbl_big_Sp %>%
    filter(Gene %in% hiTrans_Sp) %>%
    .$aATG.context %>%
    mutinfostr %>%
...
```{r mutinf_cytoribo_Sp,dependson=c("load_context_Sp","load_riboviz_Sp"),results="show",fig.height=4,fig.width=5}
ctbl_big_Sp %>%
    filter(Gene %in% cytoribolist_Sp) %>%
    .$aATG.context %>%
    mutinfostr %>%
...
```{r cytoribo_contextm2_Sp,dependson="load_context_Sp",fig.width=4,fig.height=3}

seqlogoFilterGenePos <- function(datain=ctbl_big_Sp,Genelist=hiTrans_Sp,
                                 pos=8L, nt="T") {
...
plot_grid(seqlogoFilterGenePos(Genelist=cytoribolist_Sp,pos=11L,nt="T"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Sp,pos=11L,nt="C"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Sp,pos=11L,nt="A"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Sp,pos=11L,nt="G"),
    ncol=1
)
