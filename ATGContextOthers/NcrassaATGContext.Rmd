...
ctbl_big_Nc <- read_tsv("data_in/NC12_allATGcontext.table.txt",
                        skip=1,
...
```{r kozak_inf_Nc,fig.width=4,fig.height=1}
...
entropytot(kozak_w_PFM_Nc)
# entropytot(kozak_w_PFM_Nc[,5:12])
entropytot(kozak_w_PFM_Nc[,-(10:12)])
entropytot(kozak_w_PFM_Nc[,5:9])
```

Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r entropytot(kozak_w_PFM_Nc[,5:9]) %>% round(2)`, of **wide** is `r entropytot(kozak_w_PFM_Nc[,-(10:12)]) %>% round(2)`.
...
PWMscoren <- function(seqs,pwm=kozak_n_PWM_Nc,startl=8) {
    PWMscore(seqs,pwm,startl) 
}

PWMscorew <- function(seqs,pwm=kozak_w_PWM_Nc,startl=4) {
    PWMscore(seqs,pwm,startl) 
}
...
```{r load_mitofates_Nc}
mitofates_Nc <- read_tsv("data_in/NC12_mitofates.txt",
                            comment="#",skip=1,
