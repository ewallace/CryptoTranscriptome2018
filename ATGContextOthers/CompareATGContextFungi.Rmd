...
```{r plot_inf_perbase,dependson="load_info_perbase",fig.height=4,fig.width=4}
...
Second figure here is the standout. Might need redoing so uses a colour not an alpha/transparency for higher bars. 

```{r inf_content_bygenesm4ANDm10to5,dependson="load_info_perbase",results="show",fig.height=3,fig.width=4}
# infon_perbase

infon_tblv <- bind_rows(
    infon_perbase_g %>% 
        filter(Pos >= -10, Pos <= -5) %>%
        group_by(Organism,Genes) %>%
        summarize(Infon=sum(Infon)) %>%
