...
```{r dvsaATG_framemitosumm, dependson="load_dvsa",results="show",fig.width=4.5,fig.height=4}
...
```{r dvsaATG_framemitosumm_enough, dependson="load_dvsa",results="show",fig.width=4.5,fig.height=3}
dvsaATG_framemitosummspreade <-
    dvsa_tbl %>%
    unite(d1f_mito,d1.framefac,Pred_preseq) %>%
    spread(key=d1f_mito,value=n) %>%
    mutate(d1vsaw0p1=factor(d1vsaw0p1,
...
ggplot(data=dvsaATG_framemitosummspreade %>%
           filter(enoughR=="Yes") ) +
    geom_col(aes(x=Organism,
                 y=(In_No+In_Yes)/(In_No+In_Yes+Out_No+Out_Yes),
                 fill="Yes") ) +
