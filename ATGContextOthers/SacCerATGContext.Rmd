...
cytoribolist_Sc <- read_tsv("data_out/ComplexOrthologs/cytoribosome_PTRcurated6.txt",
                        comment="#") %>%
...
kozak_all_PFM_Sc <- filter(ctbl_big_Sc,Gene %in% hiTrans_Sc) %>%
    .$aATG.context %>%
    
    str_to_upper() %>%
    consensusMatrix() 

kozak_w_PWM_Sc <- PWM(kozak_all_PFM_Sc[,3:15])
kozak_n_PWM_Sc <- PWM(kozak_all_PFM_Sc[,9:15])
```
...
Information content in bits of highly-translated consensus (excluding 6 bits from ATG), **narrow** is `r infontot(kozak_all_PFM_Sc[,5:9]) %>% round(2)`, of **wide** is `r infontot(kozak_all_PFM_Sc[,-(10:12)]) %>% round(2)`.


### Estimate information content per base across start context


```{r kozak_inf_perbase_Sc,dependson=c("kozak_consensus_Sc","cytoribo_context_Sc"),fig.width=6,fig.height=1.4,results="show"}
inf_content_perbase_Sc <- 
    tibble(i = 1:24, 
           Pos = c(-12:-1, 1:12),
        All = ctbl_big_Sc %>%
            .$aATG.context %>%
        ...
        HiTrans = ctbl_big_Sc %>%
            ...
            .$aATG.context %>%
            infonfromseqs(scope="all"),
        CytoRibo = ctbl_big_Sc %>%
            filter(Gene %in% cytoribolist_Sc) %>% 
            .$aATG.context %>%
            infonfromseqs(scope="all")
    )

write_tsv(inf_content_perbase_Sc %>%
              mutate_if(is.numeric,round3),
          "data_out/inf_content_perbase_Sc.txt")
# inf_content_perbase

plot_inf_perbase(inf_content_perbase_Sc)
```
...
scores_Sc <- 
    ctbl_big_Sc %>%
    transmute(Gene,
              aATG.scorekn = PWMscoren(str_to_upper(aATG.context),kozak_n_PWM_Sc),
              d1.scorekn   = PWMscoren(d1.context,kozak_n_PWM_Sc),
              u1.scorekn   = PWMscoren(u1.context,kozak_n_PWM_Sc)
    ) %>%
...
```{r mutinf_Sc,dependson=c("load_context_Sc","load_riboviz_Sc"),results="show",fig.height=4,fig.width=5}
mutinf_Sc <- mutinfostr(ctbl_big_Sc$aATG.context)
# mutinf_Sc

plot_mutinfo(mutinf_Sc)
```

### For highly translated genes

```{r mutinf_hiTrans_Sc,dependson=c("load_context_Sc","load_riboviz_Sc"),results="show",fig.height=4,fig.width=5}
ctbl_big_Sc %>%
    filter(Gene %in% hiTrans_Sc) %>%
    .$aATG.context %>%
    mutinfostr %>%
...
```{r mutinf_cytoribo_Sc,dependson=c("load_context_Sc","load_riboviz_Sc"),results="show",fig.height=4,fig.width=5}
ctbl_big_Sc %>%
    filter(Gene %in% cytoribolist_Sc) %>%
    .$aATG.context %>%
    mutinfostr %>%
...
```{r cytoribo_contextm2_Sc,dependson="load_context_Sc",fig.width=4,fig.height=3}

seqlogoFilterGenePos <- function(datain=ctbl_big_Sc,Genelist=hiTrans_Sc,
                                 pos=8L, nt="T") {
...
plot_grid(seqlogoFilterGenePos(Genelist=cytoribolist_Sc,pos=11L,nt="T"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Sc,pos=11L,nt="C"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Sc,pos=11L,nt="A"),
          seqlogoFilterGenePos(Genelist=cytoribolist_Sc,pos=11L,nt="G"),
    ncol=1
)
...
```{r dvsaATG_scoresn_boxplot_Sc, dependson="calc_scores_Sc", fig.width=3.5,fig.height=3.5}
scores_Sc_Cutn <- 
    scores_Sc %>% 
    select(Gene,aATG.scorekn,d1.scorekn) %>%
    filter(!is.na(d1.scorekn),Gene %in% enoughRNA_Sc) %>%
...
### dATG in frame with ATG - CHECK against dual-localized papers!

Files with high difference in narrow score, filtered for reasonable amounts of RNA, in frame.

```{r highdiffn_dvsaATG_inframe_Sc,dependson="dvsaATG_scoresn_Sc",results="show"}
highdiffn_dvsaATG_enoughinframe_Sc <- 
    highdiffn_dvsaATG_Sc %>%
    left_join( ctbl_big_Sc %>% select(Gene,d1.frame) ) %>%
    filter(Gene %in% enoughRNA_Sc, d1.frame==0) %>%
    select(Gene,aATG.scorekn,d1.scorekn)
...
```{r highdiffn_dvsaATG_outframe_Sc,dependson="dvsaATG_scoresn_Sc",results="show"}
highdiffn_dvsaATG_enoughoutframe_Sc <- 
    highdiffn_dvsaATG_Sc %>%
    left_join( ctbl_big_Sc %>% select(Gene,d1.frame) ) %>%
    filter(Gene %in% enoughRNA_Sc, d1.frame!=0) %>%
    select(Gene,aATG.scorekn,d1.scorekn)
...
```{r load_mitofates_Sc}
mitofates_Sc <- read_mitofates("data_in/SacCer_nuclear_mitofates.txt")
