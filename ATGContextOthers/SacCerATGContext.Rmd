...
This presents the renewed analysis of Saccharomyces start codon usage and context. This uses the S. cerevisiae S288C expression estimates from Csardi et al 2015, and translation estimates from Geraschenko and Gladyshev 2014.

This document was started in Aug 2018, updated through Jan 2019, and used as a template for the analyses in other fungi.
...
weinberg_riboprofiling <- read_tsv("data_in/ExpressionTranslation/Weinberg2016_SacCer_riboprofiling.txt") %>%
    dplyr::select(Gene=Gene_name,RNA=RNA,RPF = Ribo) %>%
    mutate(TE = RPF/RNA)

gerasch_riboprofiling <- read_tsv("data_in/ExpressionTranslation/Geraschenko2014_SacCer_GSE59573_Expression_Table.txt",
                    skip=3,
                    col_names = c("Gene", "unstressed_noCHX", "unstressed_1_64x_CHX", "unstressed_1_16x_CHX", "unstressed_1_4x_CHX", "unstressed_1x_CHX", "unstressed_8x_CHX", "unstressed_100x_CHX_rep1", "unstressed_100x_CHX_rep2", "oxidative_noCHX", "oxidative_1_64x_CHX", "oxidative_1_16x_CHX", "oxidative_1_4x_CHX", "oxidative_1x_CHX", "oxidative_8x_CHX", "oxidative_100x_CHX_rep1", "oxidative_100x_CHX_rep2", "heatshock_noCHX", "heatshock_1x_CHX", "complete_SD_media", "SD_no_aminoacids", "SD_no_aminoacids_plus_His_Met_Leu", "edeine_noCHX", "aurin_noCHX") )

csardi_mrna_prot <- read_tsv("data_in/ExpressionTranslation/scer-mrna-protein-absolute-estimate.txt",comment="#")

ribosum <-  gerasch_riboprofiling %>%
        transmute(GeneAlt=Gene,
                  Gene = str_sub(Gene,end=7L),
                  RPF=( unstressed_noCHX + unstressed_1_64x_CHX + unstressed_1_16x_CHX +  unstressed_1_4x_CHX + unstressed_1x_CHX ) / 5 ) %>%
    left_join(  csardi_mrna_prot %>% select(Gene=orf,Gene2=gene,RNA=mrna,Prot=prot) ) %>%
    mutate(TE = RPF/RNA)

ribosum %>%
    arrange(desc(RPF)) 

# Takes the first entry of each deduplicated gene
hiTransFirst <- ribosum %>%
    arrange(desc(RPF)) %>%
    head(n=300) %>%
    .$Gene

# Takes all entries of each deduplicated gene
hiTrans <- ribosum %>%
    arrange(desc(RPF)) %>%
    .$GeneAlt %>%
    str_split("_") %>%
...
```{r plot_TE_Weinberg,dependson="load_ribosum",fig.height=3.9,fig.width=4.1}
TE_unusual <- 
    weinberg_riboprofiling %>%
    filter( (RPF > 1.8e4) |
                (RPF > 100 & RPF > RNA * 5 ) | 
                (RNA > 150 & RPF < RNA / 10 ))


ggplot(data=weinberg_riboprofiling,aes(x=RNA,y=RPF)) +
    geom_diagline() +
...
```{r plot_TE_geraschcsardi,dependson="load_ribosum",fig.height=3.9,fig.width=4.1}
...
```{r a_hi_context_svg,fig.width=3,fig.height=0.8,dev="svg"}
ggseqlogo(data=filter(ctbl_big,Gene %in% hiTrans)$aATG.context,
              namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
    annotate(geom="text",label="S. cerevisiae",
...
```{r cytoribo_context_svg,dependson="cytoribo_context",fig.width=3,fig.height=0.55,dev="svg"}
ggseqlogo(data=filter(ctbl_big,Gene %in% cytoribolist)$aATG.context,
              namespace="TCAG") + 
    xlim(c(2.5,21.5)) + 
    theme_nothing() +
...
There are many duplicate cytoRibo genes in Sc, and in the Geraschenko dataset these are both included

Both duplicates were discarded from the Weinberg & Shah et al ribosome profiling dataset, which is why we stopped using it.
...
ggplot(data=scores) +
    ...
    scale_colour_manual(values=c("aATG"="dodgerblue",
                                 ...
                                 "uATG"="red",
                                 "dATG"="green3") ) +
    labs(x="ATG narrow Kozak score, Sc") +
...
ggplot(data=scores) +
    ...
    scale_colour_manual(values=c("aATG"="dodgerblue",
                                 ...
                                 "uATG"="red",
                                 "dATG"="green3") ) +
    labs(x="ATG wide Kozak score, Sc") +
    theme(axis.line.y=element_blank(),axis.ticks.y=element_blank(),
          axis.text.y=element_blank(),axis.title.y=element_blank(),
          legend.title=element_blank())
```
...
### uATGs are barely associated with lower absolute translation

Here we use the Weinberg ribosome profiling data, which throws out duplicated genes.

```{r uATG_RPF, dependson="calc_scores", fig.width=2.5,fig.height=3.5}
uATG_ct <- read_tsv("data_in/SacCer_allATGcontext.table.txt") %>% 
    dplyr::rename(Gene=gene) %>%
    dplyr::rename_all(str_replace,pattern="-ATG",replacement="") %>%
    dplyr::select(c("Gene",paste0("u",1:19,".posTSS"))) %>%
...
### uATGs are barely associated with lower translation efficiency
...
### uATGs weakly associated with lower translation efficiency might be over 20nt from TSS
...
```{r uATG_TE_comp,dependson=c("uATG_TE","uATGmin20_TE"),fig.width=2.5,fig.height=6}
plot_grid(uATG_TE + 
              labs(title="S. cerevisiae") + 
              theme(plot.title = element_text(face = "italic")),
          uATGmin20_TE,ncol=1,align="v",labels=c("E","F"))
```



### uATG score does not affect TE
...
ggplot(data=ribo_uct %>%
    ...
    filter(uATGCt == 1) %>%
        left_join(ctbl_big) %>%
        mutate(u1.posTSS20 =  cut(u1.posTSS,breaks=c(0,20,5000))),
...
highdiffn_dvsaATG_enoughinframe %>% print(n=15)
...
- TRM5 / YHR070W, tRNAmethyltransferase, dual-localized mito and cyto from in-frame start codons
- RSC2 / YLR357W, chromatin remodeler
- VAS1 / YGR094W, Valyl tRNA synthase, localized to cytoplasm and mitochondria
- UBP1 / YDL122W, Ubiquitin-specific protease, localizes to cytoplasm and ER
...
- RVS167 / YDR388W, Calmodulin-binding actin-associated protein
...
- TPK1 / YJL164C, cAMP-dependent protein kinase catalytic subunit
...
```{r dvsaATG_scoresdiffw_mitofates, dependson=c("calc_scores","load_mitofates"), fig.width=5,fig.height=1.8}
...
ggplot(data=scores %>% 
    ...
    scale_colour_manual(values=c("Yes"="magenta","No"="grey50"),
                        labels=mitobreaks)  + 
    labs(x="Kozak wide score, difference dATG - aATG", colour="Mito. pred") +
    theme(axis.line.y=element_blank(),axis.ticks.y=element_blank(),
          axis.text.y=element_blank(),axis.title.y=element_blank())
```

### There are no correlations between d1 score, d1 frame, and localization
...
write_tsv(dvsaATG_framemitosumm,
          "data_out/dvsaATG_framemitosumm_Sc.txt")
```
