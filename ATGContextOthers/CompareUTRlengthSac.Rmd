...
## Percentiles of length distributions
...
UTRlengths %>%
  ...
  select(c(`Organism`,`5%`,`25%`,`50%`,`75%`,`95%`))
```

# Check UTR length distributions for Arribere et al

Transcript Leader length calls in nt from Arribere and Gilbert 2013 (GSM1120728/GSM1120729). Scatter plot to compare replicates, and percentiles of replicates.

```{r read_Arribere,fig.height=3.5,fig.width=4.5,results=TRUE}
UTRlen_ScerArr <- full_join(
  read_tsv("data_in/SacCerArribereetal/GSM1120728_TLSeqBio1_LengthCalls.txt",col_names=c("Gene","Rep1")),
    read_tsv("data_in/SacCerArribereetal/GSM1120729_TLSeqBio2_LengthCalls.txt",col_names=c("Gene","Rep2"))
)

ggplot(data=UTRlen_ScerArr %>%
         group_by(Rep1,Rep2) %>%
         count(),
       aes(x=Rep1,y=Rep2)) +
  geom_diagline(colour="grey50") + 
  geom_point(aes(size=n)) +
  scale_loglog(expand=c(0.01,0.01)) + 
  scale_size_area(max_size = 3) +
  coord_equal()

UTRlen_ScerArr %>% 
  gather("Rep","Length",-Gene) %>%
  group_by(Rep) %>%
  summarize(Length=quantile_tidy(Length,na.rm=TRUE)) %>%
  unnest %>%
  spread(key=name,value=value) %>%
  select(c(`Rep`,`5%`,`25%`,`50%`,`75%`,`95%`))
```
...
```{r plot_comp,dependson=c("plot_UTR_lengths","plot_uATG_prop"),fig.width=7.5,fig.height=3.5}
